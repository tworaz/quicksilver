...
QSURLRequestContextGetter::QSURLRequestContextGetter(
...
QSURLRequestContextGetter::CreateNetworkDelegate() {
  return make_scoped_ptr(new QSNetworkDelegate).Pass();
}

scoped_ptr<net::ProxyConfigService>
QSURLRequestContextGetter::GetProxyConfigService() {
  return net::ProxyService::CreateSystemProxyConfigService(
      io_loop_->task_runner(), file_loop_->task_runner());
}

scoped_ptr<net::ProxyService> QSURLRequestContextGetter::GetProxyService() {
  // TODO(jam): use v8 if possible, look at chrome code.
  return net::ProxyService::CreateUsingSystemProxyResolver(
      proxy_config_service_.Pass(), 0, url_request_context_->net_log());
}

net::URLRequestContext* QSURLRequestContextGetter::GetURLRequestContext() {
...
scoped_refptr<base::SingleThreadTaskRunner>
    QSURLRequestContextGetter::GetNetworkTaskRunner() const {
  return BrowserThread::GetMessageLoopProxyForThread(BrowserThread::IO);
}

net::HostResolver* QSURLRequestContextGetter::host_resolver() {
