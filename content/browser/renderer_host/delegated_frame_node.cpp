...
#include "content/browser/renderer_host/chromium_gpu_helper.h"
#include "content/browser/renderer_host/stream_video_node.h"
#include "content/browser/renderer_host/yuv_video_node.h"
...
namespace {
QSGLayer* CreateLayer(QQuickItem* item) {
  QSGRenderContext* renderContext =
      QQuickWindowPrivate::get(item->window())->context;
  return renderContext->sceneGraphContext()->createLayer(renderContext);
}
}

ChromiumCompositorData::ChromiumCompositorData() {
}

ChromiumCompositorData::~ChromiumCompositorData() {
}

DelegatedFrameNode::SGObjects::SGObjects() {
}
...
public:
    ...
    QSharedPointer<QSGTexture> initTexture(bool quadIsAllOpaque, QQuickItem *view = 0);
...
static void waitChromiumSync(gfx::TransferableFence *sync)
{
    ...
    case gfx::TransferableFence::EglSync:
    {
        ...
        if (!resolved) {
            ...
            if (context->hasExtension(QByteArrayLiteral("EGL_KHR_fence_sync"))) {
...
#else
        glWaitSync_(sync->arb.sync, 0, GL_TIMEOUT_IGNORED_APPLE);
#endif
        break;
    }
}
...
QSharedPointer<QSGTexture> ResourceHolder::initTexture(bool quadNeedsBlending, QQuickItem *view)
{
    ...
    if (!texture) {
        if (m_resource.is_software) {
            ...
            QImage image(sharedBitmap->pixels(), m_resource.size.width(), m_resource.size.height(), format);
						texture.reset(view->window()->createTextureFromImage(image.copy(), QQuickWindow::TextureCanUseAtlas));
        } else {
...
void DelegatedFrameNode::commit(ChromiumCompositorData *chromiumCompositorData, cc::ReturnedResourceArray *resourcesToRelease, QQuickItem* view)
{
    ...
    for (unsigned i = 0; i < frameData->render_pass_list.size(); ++i) {
        ...
        if (pass != rootRenderPass) {
            ...
            if (!rpLayer) {
                QSGRenderContext* renderContext = QQuickWindowPrivate::get(view->window())->context;
                //rpLayer = QSharedPointer<QSGLayer>(renderContext->sceneGraphContext()->createLayer(renderContext));
                rpLayer = QSharedPointer<QSGLayer>(CreateLayer(view));
        ...
        for (; it != end; ++it) {
            ...
            case cc::DrawQuad::CHECKERBOARD: {
                ...
                // Only QSGImageNode currently supports QSGLayer textures.
								QSGRenderContext* renderContext = QQuickWindowPrivate::get(view->window())->context;
								QSGImageNode *imageNode = renderContext->sceneGraphContext()->createImageNode();
                ...
                textureNode->setTexture(initAndHoldTexture(resource, quad->ShouldDrawWithBlending(), view));
...
QSGTexture *DelegatedFrameNode::initAndHoldTexture(ResourceHolder *resource, bool quadIsAllOpaque, QQuickItem *view)
{
    ...
    m_sgObjects.textureStrongRefs.append(resource->initTexture(quadIsAllOpaque, view));
